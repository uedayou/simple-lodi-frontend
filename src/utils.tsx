...
export async function getHttpData(url:string){
  ...
  try {
    ...
    if (!res.data) return;
      ...
      for (const s of triples) {
        ...
        rdfs[s.subject.value][s.predicate.value].push({
          value:s.object.value,
          type:s.object.termType
        })
      }
      return rdfs;
    } else {
...
export function getLink(text:string) {
  if (text.match(new RegExp("^https?://.+?.(jpg|jpeg|gif|png)$","i"))) {
    ...
    const hostname = url.parse(text).hostname
    const target = hostname===(window as any).location.hostname ?
      '_self' : '_blank'
    return (
      <a href={ text } target={target} rel="noreferrer noopener">
        { shorten(text) }
      </a>);
  }
